...
const ToggleStatusButtonCP = ({ registroId, initialStatus }) => {
  ...
  const toggleStatus = async () => {
    ...
    // Solo ciudadano: ACTIVO ↔ SOLICITAR BAJA
    let nuevoStatus = status === 'ACTIVO' ? 'SOLICITAR BAJA' : 'ACTIVO';
  ...
  return (
    <button
      className="text-white my-2 bg-pink-700 hover:bg-pink-800 focus:outline-none focus:ring-4 focus:ring-pink-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-pink-600 dark:hover:bg-pink-700 dark:focus:ring-pink-900"
